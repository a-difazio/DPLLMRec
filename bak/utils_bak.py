...
def sample_function(user_train, usernum, itemnum, batch_size, maxlen, result_queue, SEED):
    ...
    def random_neq(l, r, target_item):
        """
        Repetitive sampling until the sample is not the target item.
        It can be an already interacted item.
        """
    ...
    def sample(uid):
        ...
        for i in reversed(user_train[uid][:-1]):
            seq[idx] = i
            pos[idx] = nxt
            neg[idx] = random_neq(1, itemnum + 1, nxt)          # Don't need "if nxt != 0"
            nxt = i
            idx -= 1
            if idx == -1: break
...
def evaluate(model, dataset, args):
    ...
    for u in users:
        ...
        for _ in range(100):
            ...
            while t == target_item: t = np.random.randint(1, itemnum + 1)
